...
public function generate_16x(){
$footer='<footer id="footer">
        <div class="container">
            <div class="row">
                <div class="col-sm-6">
                    &copy; 2017 Dinas Arsip dan Perpustakaan. Pemerintah Kota Depok
                </div>
                <div class="col-sm-6">
...
function get_detail($id_content){
			...
			$blog.='<div class="col-md-8">
						...
						<div class="blog-post blog-large wow fadeInLeft" data-wow-duration="300ms" data-wow-delay="0ms">
							<article>
								...
								<footer class="entry-meta">
									<span class="entry-author"><i class="fa fa-pencil"></i> <a href="#">'.$row->create_by.'</a></span>
									<span class="entry-comments"><i class="fa fa-comments-o"></i> <a href="features/detil/'.$row->fe_content_id.'">15</a></span>
								</footer>
							</article>
						</div>
					</div><!--/.col-sm-6-->';
		...
		$sql2 ="SELECT fe_content_id, fe_img_files, fe_content_title, fe_content_short_desc, fe_content_short_desc2, fe_full_content, display_start_from, fe_content_btn_label, create_by, create_date 
		FROM sys_fe_content WHERE  status_active = 'Y' AND fe_section_name='$fe_section_name' AND fe_content_id NOT IN('$content_id')  ORDER BY fe_content_sort_order DESC";

		$query2=$this->db->query($sql2);
		$blog.='<div class="col-md-4"><h4 class="entry-date" style="padding-top:40px">Another Article</h4>';
		foreach($query2->result() as $row2){
		 $img_url2 = base_url(). 'assets/images/upload/features/'.$row2->fe_img_files;
        ...
        $blog.='<div class="blog-post blog-media wow fadeInRight" data-wow-duration="300ms" data-wow-delay="100ms">
                    ...
                    <article class="media clearfix">
                        ...
                        <div class="media-body">
                            <header class="entry-header">
                                <h4 class="entry-title"><a href="'.base_url().'features/detail/'.$row2->fe_content_id.'">'.$row2->fe_content_title.'</a></h4>
                                <div class="entry-date">'.indonesian_date($row2->create_date).'</div>
                            </header>
                       </div>
                    </article>
	...
	function generate_main_page($pgid){
		$section_script='';
		$sql ="SELECT page_id, section_name, article_id FROM sys_pages WHERE page_id = '$pgid' ";
 		$query = $this->db->query($sql);
		$row=$query->row();
		if($row){
			$arr_page = explode("|", $row->section_name);
			$filteredarray = array_values( array_filter($arr_page) );
			$section_script ="";
			foreach ($filteredarray as $dt){
				$funct_nm = "generate_".$dt;
				$script=get_section_script($dt);
				if($dt=='A'){
					$section_script.=$this->get_detail($row->article_id);
				}

				if(method_exists('Frontpage_model',$funct_nm)){
					$section_script.=$this->$funct_nm($script, $dt);
				}
			}
		}			
