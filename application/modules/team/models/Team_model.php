...
class Team_model extends CI_Model{
	...
	function get_detail($id_content){
		...
		$row=$query->row();
		if(isset($row)){
			$img_url = base_url(). 'assets/images/upload/team/'.$row->fe_img_files;
							...
							<article>
								<header class="entry-header">
									<div class="entry-thumbnail">
										<img class="img-responsive" style="height:30%;" src="'.$img_url.'" alt="">
									</div>
		...
		foreach($query2->result() as $row2){
		 $img_url2 = base_url(). 'assets/images/upload/team/'.$row2->fe_img_files;
        ...
        $blog.='<div class="blog-post blog-media wow fadeInRight" data-wow-duration="300ms" data-wow-delay="100ms">
                    ...
                    <article class="media clearfix">
                        ...
                        <div class="media-body">
                            <header class="entry-header">
                                <h4 class="entry-title"><a href="'.base_url().'team/detail/'.$row2->fe_content_id.'">'.$row2->fe_content_title.'</a></h4>
