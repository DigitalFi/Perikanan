...
function index(){
	...
	$this->load->library('pagination');
	$count = $this->Pages_model->pageListingCount($searchText);
	$returns = $this->paginationCompress("wbadmin/pages/", $count, 1 );

	$data = $this->load_user_info('pages');  
	$data['after_del'] ='0';
	$data['data_section']=  $this->Pages_model->get_page_section(); 
	$data['pages'] = $this->Pages_model->page_listing($searchText, $returns["page"], $returns["segment"]);
...
function page_list(){
	...
	$this->load->library('pagination');
	$count = $this->Pages_model->pageListingCount($searchText);
	$returns = $this->paginationCompress("pages/page_list/", $count, 2, $searchText );

	echo "keyword ". $returns['keyword'];
