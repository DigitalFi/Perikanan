...
*/			$team.='</div>
	...
	function generate_14($script_section, $section_name){//blog

		$blog='';
		$sql ="SELECT fe_content_id, fe_img_files, fe_content_title, fe_full_content, fe_content_short_desc, fe_content_short_desc2, 
		fe_content_btn_url, display_start_from, fe_content_btn_label, create_by, create_date FROM sys_fe_content WHERE fe_section_name='$section_name' AND status_active = 'Y'  ORDER BY fe_content_sort_order DESC  limit 0,1";
		...
		if(isset($row)){
			...
			$content_article ='....';
			if($row->fe_full_content!=''){
				$content_article = strip_tags_content(substr($row->fe_full_content,0,341));
			}
			...
			$blog.='<div class="col-sm-6">
						<div class="blog-post blog-large wow fadeInLeft" data-wow-duration="300ms" data-wow-delay="0ms">
							<article>
								...
								$blog.=$content_article.'<br>';
								$blog.='<a class="btn btn-primary" href="blog/detail/'.$row->fe_content_id.'">'.$button_label.'</a>
								</div>
                    ...
                    <article class="media clearfix">
                        ...
                        <div class="media-body">
                            <header class="entry-header">
                                ...
                                <h2 class="entry-title"><a href="blog/detail/'.$row2->fe_content_id.'">'.$row2->fe_content_title.'</a></h2>
                            </header>
	...
	function generate_15($contact_script, $section_name){//contact
		$row_section = get_section_info_by_name(strtolower($section_name));
		$img_url = base_url(). 'assets/images/upload/'.$row_section->image_section;
...
/*		$sql_item2 = "SELECT files_name, file_desc FROM tbl_gallery_files WHERE content_id = '$content_id' ORDER BY img_gallery_id";
	...
	return $gallery;
	 ...
	 function generate_page_section_ori(){
