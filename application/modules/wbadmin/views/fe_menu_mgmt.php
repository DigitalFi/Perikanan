	...
	function get_data(id_menu, menu_label, lvl){
		...
		$.post(url, {
			...
			$.each(obj, function() {
				...
				//$("#cbo_data").prepend("<option value='' >&nbsp;</option>");
				$('#cbo_data').val(this['link_menu']).trigger('chosen:updated');
				$('#cbo_section').val(this['section_id']).trigger('chosen:updated');
             });
				//$("#comment").html(result);
        });
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
	  <?php
       ...
       echo form_open("wbadmin/update_sort_order_fe", $attributes);?>
        ...
        <div class="modal-body">
            ...
            <input type="hidden" id="sv_mode" name="sv_mode" />
              ...
              <div class="form-group-sm" id="status_container" style="width:40%">
               ...
               <div class="form-group-sm" id="status_container" style="width:40%">
               <label for="status_active">Use page will use template:<span id="lbl_sec_name" data-toggle="tooltip" class="badge bg-danger" title="Article atau detail content yang akan diakses melalui menu ini. Ketik keyword untuk pencarian content lebih cepat" ><i>i</i></span></label>
                       <div id="list_article_container">
                        <?php echo $dt_cbo_section;                       //$attributes = 'class="form-control chosen-select" id="section"';
                       // echo form_dropdown('pages', $dt_cbo_article, set_value('pages'), $attributes); 
                        ?>
                </div>
              </div>
